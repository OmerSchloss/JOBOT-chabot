...
def load_JobIsJob_jobs_div(job_title, location, job_type):
    ...
    job_elems = driver.find_elements(By.CSS_SELECTOR, 'ul.list2 > li')

    for job in job_elems:
        job_dic = {}
        job_name = job.find_element(By.CSS_SELECTOR, ".title").text
        ...
        if job_link is not None:
            try:
                ...
                job_description = driver2.find_element(By.CSS_SELECTOR, ".description-text").text
    ...
    for job_title in job_titles:
        for job_location in job_locations:
            new_jobs_list, num_listings = find_jobs_from(
                'JobIsJob', job_title, job_location, job_type, desired_characs)
...
if __name__ == "__main__":
    desired_characs = ['titles', 'companies', 'links', 'date_listed']
    jobs_list = find_jobs_from('JobIsJob', 'data scientist', 'alabama', 'fulltime', desired_characs)
    print(jobs_list)
