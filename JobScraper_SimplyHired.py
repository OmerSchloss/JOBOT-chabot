...
def find_jobs_from(website, job_title, location, job_type, desired_characs):
    ...
    The arguments it takes are:
        - Website: to specify which website to search (options: 'SimplyHired' or 'CWjobs')
        - Job_title
        - Location
...
def find_job_offers(job_titles, job_locations, job_type):
    # job_data = scrape_simply_hired_jobs(job_titles[0], job_locations[0], 1)
    ...
    for job_title in job_titles:
        for job_location in job_locations:
            new_jobs_list, num_listings = find_jobs_from(
                'SimplyHired', job_title, job_location, job_type, desired_characs)
...
if __name__ == "__main__":
    ...
    jobs_list,num = find_jobs_from(
        'SimplyHired', 'data scientist', 'alabama', 'fulltime', desired_characs)
