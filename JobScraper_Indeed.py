...
def find_jobs_from(website, job_title, location, job_type):
    if website == 'Indeed':
        jobs_list, num_listings = load_indeed_jobs_div(job_title, location, job_type)
...
def load_indeed_jobs_div(job_title, location, job_type):
    getVars = {'q': job_title, 'l': location, 'sc': job_type, 'fromage': 'last', 'sort': 'date'}
    ...
    job_elems = driver.find_elements(By.CSS_SELECTOR, 'ul.jobsearch-ResultsList > li > div.cardOutline')
    ...
    for job in job_elems:
        ...
        if job_link is not None:
            ...
            job_description = driver2.find_element(By.CSS_SELECTOR, "#jobDescriptionText").text
...
def find_job_offers_in_Indeed(job_titles, job_locations, job_type):
    ...
    for job_title in job_titles:
        for job_location in job_locations:
            new_jobs_list, num_listings = load_indeed_jobs_div( job_title, job_location, job_type)
            if num_listings > 0:
...
def find_job_offers_async_in_Indeed(job_titles, job_locations, job_type):
    ...
    thread = threading.Thread(target=find_job_offers_in_Indeed, args=(job_titles, job_locations, job_type))
    thread.start()
...
if __name__ == "__main__":
    jobs_list = find_jobs_from('Indeed', 'data scientist', 'alabama', 'fulltime')
    print(jobs_list)
