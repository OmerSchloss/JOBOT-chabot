...
def find_jobs_from(website, job_title, location, job_type):
    if website == 'SnagaJob':
        jobs_list, num_listings = load_SnagaJob_jobs_div(
...
def load_SnagaJob_jobs_div(job_title, location, job_type):
    ...
    url = ('https://www.snagajob.com/search?' +
    ...
    for job in job_elems:
        ...
        try:
            ...
            job_name = job.find_element(
                By.CSS_SELECTOR, 'div.job-card--title > h2.heading-md > span.heading-100').text
            job_dic['job_name'] = job_name
            company_name = job.find_element(
                By.CSS_SELECTOR, 'div.absolute > span').text
            ...
            if job_link is not None:
                ...
                try:
                    ...
                    job_description = driver2.find_element(
                        By.CSS_SELECTOR, "div.job-description > div > div").text
...
def find_job_offers_in_SnagaJob(job_titles, job_locations, job_type):
    ...
    for job_title in job_titles:
        for job_location in job_locations:
            new_jobs_list, num_listings = load_SnagaJob_jobs_div(
...
def find_job_offers_thread_in_SnagaJob(job_titles, job_locations, job_type):
    global thread
    # Start a new thread for find_job_offers
    thread = threading.Thread(target=find_job_offers_in_SnagaJob, args=(
        job_titles, job_locations, job_type))
    thread.start()


def get_job_offers_in_SnagaJob(exit=False):
...
if __name__ == "__main__":
    jobs_list = find_jobs_from(
        'SnagaJob', 'data scientist', 'alabama', 'fulltime')
    print(jobs_list)
