...
def find_jobs_from(website, job_title, location, job_type):
    if website == 'Google':
        jobs_list, num_listings = load_Google_jobs_div(
...
def load_Google_jobs_div(job_title, location, job_type):
    getVars = {'employment_type': job_type,
               'location': location, 'q': job_title, 'sort_by': 'date'}
    ...
    job_elems = driver.find_elements(
        By.CSS_SELECTOR, 'ol.gc-p-results__results-list>li')
    for job in job_elems:
        ...
        try:
            ...
            job_name = job.find_element(
                By.CSS_SELECTOR, "div.gc-card__header > h2.gc-card__title").text
            job_dic['job_name'] = job_name
            company_name = job.find_element(
                By.CSS_SELECTOR, "ul.gc-job-tags > li.gc-job-tags__team > span").text
            ...
            if job_link is not None:
                try:
                    ...
                    job_description = driver2.find_element(
                        By.CSS_SELECTOR, ".gc-job-detail__section--description").text
...
def find_job_offers_in_Google(job_titles, job_locations, job_type):
    ...
    for job_title in job_titles:
        for job_location in job_locations:
            new_jobs_list, num_listings = load_Google_jobs_div(
                job_title, job_location, job_type)  # type: ignore
            if num_listings > 0:
...
def find_job_offers_thread_in_Google(job_titles, job_locations, job_type):
    global thread
    # Start a new thread for find_job_offers
    thread = threading.Thread(target=find_job_offers_in_Google, args=(
        job_titles, job_locations, job_type))
    thread.start()


def get_job_offers_in_Google(exit=False):
